...
from svecalign.vecalign.dp_core import make_dense_costs, score_path, sparse_dp, make_sparse_costs, dense_dp

logger = logging.getLogger('vecalign')  # set up in vecalign.py


def make_norm1(vecs0: np.ndarray):
    """
    make vectors norm==1 so that cosine distance can be computed via dot product
    :param vecs0: shape = (num overlaps, num sentence, embed dim=1024)
    """
